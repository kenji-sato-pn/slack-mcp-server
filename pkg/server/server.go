...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger, enabledTools []string) *MCPServer {
	...
	if !provider.IsBotToken() && shouldAddTool(ToolDraftsCreate, enabledTools, "SLACK_MCP_ADD_MESSAGE_TOOL") {
		s.AddTool(mcp.NewTool(ToolDraftsCreate,
			...
			mcp.WithString("text",
				mcp.Required(),
				mcp.Description("Draft message text. Supports markdown formatting: **bold**, _italic_, `code`, ~~strike~~, [text](url), bullet lists (- item), ordered lists (1. item), > blockquotes, ```code blocks```. Converted to Slack rich_text format."),
			),
	...
	if !provider.IsBotToken() && shouldAddTool(ToolDraftsUpdate, enabledTools, "SLACK_MCP_ADD_MESSAGE_TOOL") {
		s.AddTool(mcp.NewTool(ToolDraftsUpdate,
			...
			mcp.WithString("text",
				mcp.Required(),
				mcp.Description("New draft message text. Supports markdown formatting: **bold**, _italic_, `code`, ~~strike~~, [text](url), bullet lists (- item), ordered lists (1. item), > blockquotes, ```code blocks```. Converted to Slack rich_text format."),
			),
			mcp.WithString("schedule_at",
